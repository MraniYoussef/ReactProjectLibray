import { useState } from "react";
import './BookReservation.css';
import { DateTimePickerComponent} from '@syncfusion/ej2-react-calendars';

function BookReservation({details, status, deleteBookReservationCallBack, saveBookReservationCallBack}){
    const [isModeEdit, setIsModeEdit] = useState(false);
    const [data, setData] = useState({
        ...details,
    });
    const [loading, setLoading] = useState(false);
    const handleOnEditClick = () => {
        setIsModeEdit(!isModeEdit)
    }
    const handleOnChange = e => {
        setData({
            ...data, [e.target.name] : e.target.value, 
        })
    }
    const handleOnDeleteClick = () => {
        console.log(details);
        deleteBookReservationCallBack(details.idBookReservation)
    }
    
    const handleOnSaveClick = async  () => {
        setLoading(true);
         await saveBookReservationCallBack(data);
             setLoading(false);
             setIsModeEdit(false)
         }
    
    return (
        
        
        <div className="BookReservation">
                 <img src= "data:image/jpeg;base64," />
                 {loading ? 
                <div>Loading...</div> : (
                    <div>
                    {isModeEdit ? (
                        <>
                            <label for='idMember'>Id Member : </label>
                            <input name='idMember' value={data.member.idMember} onChange={handleOnChange} />
                            <label for='dateReservation'>Date Reservation : </label>
                            <DateTimePickerComponent name='dateReservation' value={data.dateReservation} onChange={handleOnChange}></DateTimePickerComponent>
                            <label for='status'>BookReservation Status : </label>
                            <select defaultValue={status && status[0]} name='status' onChange={handleOnChange}>{status?.map(status => <option key={status} value={status}>{status}</option>)}</select ><br/>
                            <label for='dateTermination'>Date Termination : </label>
                            <DateTimePickerComponent name='dateTermination' value={data.dateTermination} onChange={handleOnChange}></DateTimePickerComponent>
                            <label for='returnDate'>Return Date : </label>
                            <DateTimePickerComponent name='returnDate' value={data.returnDate} onChange={handleOnChange}></DateTimePickerComponent>
                            <label for='book'>Id Book : </label>
                            <input name='book' value={data.book.idBook} onChange={handleOnChange} />
                            
                            <button onClick={handleOnSaveClick}>Save</button> 
                        </>
                          ) : (
        <>                                                                      
           <label> Id Reservation : {details.idBookReservation} </label><br/>
           <label> Id : {details.member.idMember} Name :  {details.member.lastName}</label><br/>
           <label> Date Reservation : {details.dateReservation} </label><br/>
           <label> Book Reservation Status : {details.status} </label><br/>
           <label> Date Termination : {details.dateTermination} </label><br/>
           <label> Return Date {details.returnDate} </label> <br/>          
           <label> Book {details.book.idBook} Title :  {details.book.title} </label><br/>
           <button onClick={handleOnDeleteClick}>Delete</button> 
                          
        </>
            )}  
            <button onClick={handleOnEditClick}>Edit</button> 
        </div>
         )} 
              
    </div>
    )
}
export default BookReservation;